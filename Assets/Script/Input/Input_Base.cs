...
public class Input_Base
{
    ...
    public Queue<InputType> mouseQueBase = new Queue<InputType>();

    public Queue<Vector3> moveQueBase = new Queue<Vector3>();
    public void inputEvent() 
    {
        if (Input.GetMouseButton(0)) mouseQueBase.Enqueue(InputType.Click);//mouseClick
        if (Input.GetMouseButtonUp(0)) mouseQueBase.Enqueue(InputType.Release);//mouseClickUp
        if (Input.GetMouseButtonDown(0)) mouseQueBase.Enqueue(InputType.Hold);//mouseClickDown
